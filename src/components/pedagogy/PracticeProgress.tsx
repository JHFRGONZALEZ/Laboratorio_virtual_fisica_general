...
export const PracticeProgress: React.FC<PracticeProgressProps> = ({ phaseIndex, phaseCount, phaseLabel, measurements, analysisReady, reportReady, accentClass = 'bg-blue-600' }) => {
  ...
  return <section className="container mx-auto px-4 pb-2"><div className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2 text-sm font-semibold text-gray-700"><Award size={17} className="text-yellow-500" /> Progreso de la práctica</div><span className="text-xs font-medium text-gray-500">Fase {phaseIndex + 1}/{phaseCount}: {phaseLabel} · {unlocked}/4 logros</span></div><div className="mb-3 h-2 overflow-hidden rounded-full bg-gray-100"><div className={`${accentClass} h-full rounded-full transition-all duration-500`} style={{ width: `${((phaseIndex + 1) / phaseCount) * 100}%` }} /></div><div className="flex flex-wrap gap-2">{achievements.map(item => <span key={item.label} className={`inline-flex items-center gap-1 rounded-full border px-2 py-1 text-xs ${item.unlocked ? 'border-yellow-200 bg-yellow-50 text-yellow-800' : 'border-gray-200 bg-gray-50 text-gray-400'}`}>{item.icon}{item.label}{item.unlocked ? ' ✓' : ' 🔒'}</span>)}</div></div></section>;
};
